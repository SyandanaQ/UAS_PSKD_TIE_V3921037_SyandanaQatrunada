...
demo = cv2.imread("C:/image.jpg", 0)
r, c = demo.shape
key = np.random.randint(0, 256, size=(r, c), dtype=np.uint8)  # Generate random key image
cv2.imwrite("C:\Users\PAVILION GAMING\Pictures\kunci1", key)   # Save key image
...
cv2.imwrite("C:/image_asli_enkripsi.png", encryption)  
   # Save the encrypted image
decryption = cv2.bitwise_xor(encryption, key)  # decrypt
cv2.imwrite("C:/image_asli_dekripsi.png", decryption) # Save the decrypted image
